...
class AuthAccountantController extends Controller
{
    ...
    public function __construct() {
        $this->middleware('auth:accountant', ['except' => ['login']]);
    }
    /**
    ...
    public function login(Request $request){
        ...
        if ($token = auth()->guard('accountant')->attempt($validator->validated())) {
            if(! $this->check_email_verification($request->email)){
                $admin = Accountant::where('email', $request->email)->first();
        ...
        $email = Accountant::where('email', $request->email)->get();
        if(! $email->isEmpty()){
    ...
    private function check_email_verification($email){
        $user = Accountant::where('email', $email)->whereNull('email_verified_at')->get();
        if(!$user->isEmpty()){
            return false;
        }
        return true;
...
//    }
    ...
    public function userProfile() {
        return response()->json(auth()->guard('accountant')->user());
    }
    /**
